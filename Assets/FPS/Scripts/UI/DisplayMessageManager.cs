...
namespace Unity.FPS.UI
{
    public class DisplayMessageManager : MonoBehaviour
    {
        ...
        {
            Debug.Log(
                $"DisplayMessageEvent received | Message: {evt.Message} | Delay: {evt.DelayBeforeDisplay}"
            );

            NotificationToast notification =
                Instantiate(
                    MessagePrefab,
                    DisplayMessageRect.transform
                ).GetComponent<NotificationToast>();

            m_PendingMessages.Add(
                (Time.time, evt.DelayBeforeDisplay, evt.Message, notification)
            );
        }
